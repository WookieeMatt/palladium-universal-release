...
- **Who goes first: the GM decides** (p.84). Five icon buttons at the top of the tracker (GM; hover for what each does). Only the combatants on the tracker take part:
  - <i class="fa-solid fa-user-shield"></i> **Friendly side** / <i class="fa-solid fa-skull"></i> **Hostile side** first: ordered by side (Friendly tokens, or player characters without a token, against Hostile and Neutral tokens), alternating from the chosen side, random within each side; extras go at the end.
  - <i class="fa-solid fa-dice-d20"></i> **Showdown**: the order is cleared and everyone rolls 1D20 + Initiative bonus. Each player online gets a **Roll Initiative** popup for their own combatants (or **Later**: the chat card has the same button); the GM gets one **Roll NPCs** button for every NPC and anyone whose player is offline. The tracker sorts itself; **ties re-roll**.
  - <i class="fa-solid fa-user-secret"></i> **Ambush**: pick from one dropdown (a Sneak Attack or Surprise Attack started it): *Friendly side (all)*, *Hostile side (all)*, or one combatant: they have the Initiative at the top; everyone else rolls as in a Showdown.
  - <i class="fa-solid fa-bolt"></i> **Sudden Violence**: pick who started it from a dropdown: they have the Initiative; everyone else rolls.
  - The button used last stays lit. Initiative is kept between rounds for the whole combat; anyone can still cut in line with ⇈ / ⇊.
