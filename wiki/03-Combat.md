...
- **Who goes first: the GM decides** (p.84). At the top of the tracker the GM has **Friendly first** and **Hostile first**: the combatants are ordered by side (Friendly tokens, or player characters without a token, against Hostile and Neutral tokens), alternating from the chosen side, in random order within each side; extras go at the end. Anyone can still cut in line with ⇈ / ⇊.
- Or a **Showdown**: roll Initiative as usual, 1D20 + Initiative bonus, **once** at the start of combat and kept between rounds. **Ties re-roll** automatically.
...
- **Move Action** (p.85, once per round): the walking figure next to the dots. Click it when moving (owner or GM): it lights up and spends an action; click again to undo. It resets each round. Changing posture (standing up) and object actions are separate.
