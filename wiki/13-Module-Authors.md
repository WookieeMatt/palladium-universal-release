...
| Welcome | `showRecommendedModules()` (posts the GM's module card), `showPlayerWelcome(user)` (the players' welcome, whispered to that user), `recommendedModules()` (each module's status); the list is `CONFIG.PALLADIUM.RECOMMENDED_MODULES` |
