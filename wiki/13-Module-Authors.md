...
| Reset | `resetCharacter(actor, {confirm})` (starts creation over; counts a player's reset), `resetsLeft(actor)`; hook `palladium.resetCharacter(actor, {items, by})` |
| Welcome | `showRecommendedModules()` (posts the GM's module card), `showPlayerWelcome(user)` (the players' welcome, whispered to that user), `resetWelcome()` (GM: both cards show again on everyone's next load), `recommendedModules()` (each module's status); the list is `CONFIG.PALLADIUM.RECOMMENDED_MODULES` |
| Combat | `strikeBonus(actor, weapon, mode)`, `rollAttack(actor, weapon, mode, {weather})` (black powder: `weather` skips the prompt), `askPowderWeather(weapon)`, `scenePowderWeather(scene)`, `setScenePowderWeather(key, scene)` (GM), `sceneWeatherDialog(scene)` (GM), `rollDamage(actor, weapon, options)`, `applyDamage(actor, amount, {strike, mode})`, `rollManeuver(actor, key)`, `attackModes(actor, weapon)` (the weapon's attack modes: `{key, label, bonus}`), `rollCombat(actor, key)` (a key of `COMBAT_ROLLS`: initiative, strike, parry, dodge, rollImpact, pullPunch, disarm; same as the sheet's buttons), `rollDefense(defender, key, attack)`, `rollHorrorFactor(actor, hf)`, `spendActions(actor, count)` |
| Magic | `castSpell(actor, spell)`, `newDay(actor)`, `rollMagicAbility(actor, index)` (the tradition's automatic percentile abilities), `rollChangeSave(actor)`, `usePsionic(actor, power)` |
...
| Initiative & sides | `initiativeDialog(combat)`, `startShowdown(combat, {first, mode})`, `rollShowdown(combat)`, `combatantSide(combatant)`, `setCombatantSide(combatant, "friendly"|"hostile")`, `toggleCombatantSide(combatant)`, `orderBySide(combat, side)` (alternate sides), `shiftInitiative(combatant, gain)`, `hasMoved(combatant)`, `toggleMove(combatant)`, `combatantActions(combatant)` |
